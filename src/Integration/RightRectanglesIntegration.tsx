...
const RightRectanglesIntegration: React.FC = () => {
  ...
  const integrate = () => {
    ...
    for (let i = 1; i <= n; i++) {
      const x = a + i * step;
      const y = func(x);
      newRectanglesData.push({ x: x - step, y: 0 }, { x: x - step, y }, { x, y }, { x, y: 0 });
    }
  ...
  return (
    <Card title="Метод правых прямоугольников" style={{ width: '100%', maxWidth: '800px', margin: '0 auto' }}>
      {/* Теория */}
      <Typography style={{ textAlign: 'left', padding: '0 20px' }}>
        ...
        <Paragraph>
          ...
          <MathJax>{`\\[ S_i = f(x_{i+1}) \\cdot h \\]`}</MathJax>
          где:
          <ul>
            <li>
            ...
            <li>
              <span> <MathJax inline dynamic>{`\\( f(x_{i+1}) \\)`}</MathJax> — значение функции в правой точке отрезка </span>
            </li>
            <li>
        ...
        <Paragraph>
          ...
          <MathJax>{`\\[ I \\approx \\sum_{i=0}^{n-1} S_i = h \\left( f(x_1) + f(x_2) + \\dots + f(x_n) \\right) \\]`}</MathJax>
        </Paragraph>
      </Typography>
